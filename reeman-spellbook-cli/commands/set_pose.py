...
def set_reeman_pose(
    ...
    *,
    ...
    """
    Reeman FlyBoat REST API localization.

    Uses POST /cmd/reloc_pose.
    Reeman expects:
        {"x": float, "y": float, "theta": float}

    ori/theta is in radians.
    z and adjust_position are ignored for Reeman but kept for compatibility.
    """
    payload = {
        "x": float(x),
        "y": float(y),
    ...
    try:
        resp = requests.post(
            f"{_robot_base_url()}/cmd/reloc_pose",
            json=payload,
...
def set_openapi_pose(
    ...
    *,
    ...
    """POST /chassis/pose — localize on current map."""
    payload = {
...
def set_pose(x: float, y: float, ori: float, *, z: float = 0.0, adjust_position: bool = True, timeout: float | None = None,) -> bool:
...
if __name__ == "__main__":
    ...
    #     ok = set_pose(pick["x"], pick["y"], pick["ori"], adjust_position=adj_in)
        ori = pick.get("ori", pick.get("theta", 0.0))
        ok = set_pose(
            pick["x"],
            pick["y"],
            ori,
            adjust_position=adj_in,
        )
        print("OK" if ok else "FAILED")
        sys.exit(0 if ok else 1)
